  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <HeadlessUiModal.BorderedContent className="flex flex-col gap-4">
        <Typography variant="sm" weight={700} className="text-white border border-Gray p-2 rounded-[.350rem]">
          {i18n._(
        ...
        {tokens.map((token) => {
          return (
            <div key={'import' + token.address} className=".token-warning-container flex flex-col gap-4 border border-Gray p-2 rounded-[.350rem] bg-ternary">
                ...
                <div className="flex flex-col">
                  <div className="sm:flex gap-4 sm:items-center">
                    <Typography variant="lg" weight={700}>
                      <ExternalLink href={getExplorerLink(chainId, token.address, 'address')} color="green">
                        {shortenAddress(token.address)}
                      </ExternalLink>
                    </Typography>
                    {importList !== undefined ? (
                      <Chip icon={importList.logoURI} color="primary" size="sm" label={importList.name} />
                    ) : (
                  ...
                  <Typography variant="xs" className="mt-1" weight={700} component="span">
                    {token.symbol}{' '}
      ...
      <Button
        ...
        className=".token-dismiss-button h-10 rounded-[.350rem] text-white"
      >
        {i18n._(t`Import`)}
      </Button>
    </div>
