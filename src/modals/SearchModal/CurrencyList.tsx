  ...
  return (
    <div
      ...
      className={classNames(
        ...
        `flex items-center w-full hover:bg-field/80 px-4 py-2 token-${currency?.symbol}`
      )}
      style={style}
...
const CurrencyList: FC<CurrencyList> = ({ currencies, otherListTokens, className }) => {
  ...
  return (
    <div
      ...
      className={currencies.length ?`flex flex-col flex-1 m-1 flex-grow min-h-[8rem] maxMd:min-h-[7rem] overflow-hidden h-full divide-y divide-dark-800`:`flex flex-col flex-1 m-1 flex-grow overflow-hidden h-full divide-y divide-dark-800`}
    >
      <AutoSizer>
        {/*@ts-ignore TYPE NEEDS FIXING*/}
        {({ height, width }) => (
          <List className={className} height={height} width={width} itemCount={itemData.length} itemSize={56}>
            {Row}
          </List>
        )}
      </AutoSizer>
