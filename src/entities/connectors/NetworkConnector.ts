...
class MiniRpcProvider implements AsyncSendable {
  ...
  public readonly clearBatch = async () => {
    ...
    json.forEach((item: any) => {
      ...
      if ('error' in item) {
        reject(new RequestError(item?.error?.message, item?.error?.code, item?.error?.data))
      } else if ('result' in item && resolve) {
        resolve(item)
      } else {
        reject(new RequestError(`Received unexpected JSON-RPC response to ${method} request.`, -32000, item))
      }
    })
  }
