...
const AccountDetails: FC<AccountDetailsProps> = ({
  ...
  const connectorName = useMemo(() => {
    ...
    return (
      <Typography variant="xs" weight={700} className="text-Gray-500">
        Connected with {name}
      </Typography>
    )
  }, [connector])
      ...
      <div className="space-y-3">
        ...
        <HeadlessUiModal.BorderedContent className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            {connectorName}
            <Button variant="outlined" color="green" size="xs" onClick={deactivate}>
              {i18n._(t`Disconnect`)}
            </Button>
          </div>
        ...
        <HeadlessUiModal.BorderedContent className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            ...
            <Button variant="outlined" color="green" size="xs" onClick={clearAllTransactionsCallback}>
              {i18n._(t`Clear all`)}
            </Button>
          </div>
