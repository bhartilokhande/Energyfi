...
const ListPanel = ({ header, items, footer, className = '' }: ListPanelProps) => {
  return (
    <div className={classNames(className, 'flex flex-col')}>
      {header && <div className="overflow-hidden border rounded-t border-Gray">{header}</div>}
      {items && (
        <div
          className={classNames(
            header ? '' : 'border-t rounded-t',
            footer ? 'bg-ternary/90' : 'border-b rounded-b bg-ternary',
      ...
      {footer && <div className="overflow-hidden border rounded-b bg-ternary border-Gray">{footer}</div>}
    </div>
  )
}
