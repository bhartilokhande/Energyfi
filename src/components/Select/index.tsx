  ...
  return (
    ...
    <div className="relative" ref={node} onClick={toggle}>
      <div className="shadow-md z-[2] relative flex border border-dark-800 bg-dark-900 h-[38px] rounded-md divide-x divide-dark-800">
        <div className="text-sm text-white flex items-center pl-3 min-w-[80px] font-medium">{value}</div>
        <div className="flex items-center justify-center w-9 font-bold text-white">
...
export const NeonSelectItem: FC<NeonSelectItemProps> = ({ onClick, value, children }) => {
  return (
    <div
      ...
      className="text-white flex w-full cursor-pointer hover:text-white px-3 py-1.5 text-md"
    >
      {children}
    </div>
  )
