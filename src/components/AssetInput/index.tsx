    ...
    header = (
      ...
      >
        <div className="flex gap-0.5 items-center">
          <Typography id={props.id} variant="h3" weight={700} className="text-white">
  ...
  if (currency) {
    content = (
      ...
      >
        {currencyLogo && (
          <div>
            <CurrencyLogo currency={currency} size={size === 'md' ? 40 : 35} className="!rounded-full" />
          </div>
        )}
        <div className="flex flex-col flex-grow">
          <Typography variant="h3" weight={700} className="w-full flex overflow-hidden">
            <NumericalInput
              disabled={disabled}
              value={value || ''}
            ...
            />
            {isDesktop && (
              <span className="text-xl pointer-events-none text-white/60">
                {currency?.symbol}
              </span>
            )}
          </Typography>
          <Typography
            ...
            variant="xs"
            className={error ? 'text-red' : usdcValue && value ? 'text-green ml-3 py-1' : 'text-low-emphesis ml-3 py-1'}
          >
        ...
        {error ? (
          ...
          showMax && (
            <Button size="xs" variant="outlined" className="!border" onClick={() => onMax()}>
              Max
            </Button>
          )
        )}
...
const AssetInputPanelBalance: FC<AssetInputPanelBalanceProps> = ({ balance, onClick, spendFromWallet = true, id }) => {
  ...
  let icon = (
    <WalletIcon width={16} height={14} className={classNames(balance ? 'text-white' : 'text-low-emphesis')} />
  )
  if (!spendFromWallet) {
    icon = (
      <BentoboxIcon
        width={16}
        height={16}
        className={classNames(balance ? 'text-white' : 'text-low-emphesis', 'truncate')}
      />
    )
  }
  ...
  return (
    <div className={classNames(error ? 'bg-red/10' : '', 'flex justify-between py-2 px-3')}>
      <div className="flex items-center gap-1.5 mr-1">
        {icon}
        <Typography variant="sm" className={classNames(balance ? 'text-white' : 'text-low-emphesis')}>
          {i18n._(t`Balance:`)}
        </Typography>
      </div>
      <Typography
