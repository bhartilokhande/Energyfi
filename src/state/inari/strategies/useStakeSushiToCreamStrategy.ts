...
export const GENERAL = (i18n: I18n): StrategyGeneralInfo => ({
  name: i18n._(t`ENERGYFI → Cream`),
  steps: [i18n._(t`ENERGYFI`), i18n._(t`xENERGYFI`), i18n._(t`Cream`)],
  zapMethod: 'stakeEnergyFiToCream',
  unzapMethod: 'unstakeEnergyFiFromCream',
  description: i18n._(
    t`Stake ENERGYFI for xENERGYFI and deposit into Cream in one click. xENERGYFI in Cream (crXENERGYFI) can be lent or used as collateral for borrowing.`
  ),
  inputSymbol: i18n._(t`ENERGYFI`),
  outputSymbol: i18n._(t`xENERGYFI in Cream`),
})
...
const useStakeSushiToCreamStrategy = (): StrategyHook => {
  ...
  const { execute, setBalances, ...baseStrategy } = useBaseStrategy({
    id: 'stakeEnergyFiToCreamStrategy',
    general,
    tokenDefinitions,
  })
