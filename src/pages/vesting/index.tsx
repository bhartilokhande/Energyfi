          ...
          <div className="grid col-span-12 gap-4 md:col-span-9">
            ...
            <DoubleGlowShadow className="max-w-[100%]">
              <div className="grid gap-8 p-5 border-2 rounded bg-dark-900 border-dark-700">
                ...
                <InariButton color="btn_primary" className="font-bold">
                  Execute
                </InariButton>
        ...
        <div className="relative w-full p-4 overflow-hidden rounded bg-dark-900">
          ...
          >
            <>
              ...
              <ExternalLink href="https://snapshot.org/#/Energyfi/proposal/QmPwBGy98NARoEcUfuWPgzMdJdiaZub1gVic67DcSs6NZQ">
                SIMP3
              </ExternalLink>
              .
              <br />
              ...
              <ExternalLink href="https://forum.EnergyFiswapclassic.org/t/simp-3-vesting-and-the-future-of-EnergyFiswap/1794">
              ...
              <ExternalLink href="https://github.com/EnergyFiswap/Energyfi-vesting">GitHub</ExternalLink>
            </>
          </div>
        </div>
      </div>
...
const ProtocolVesting = () => {
  ...
  useEffect(() => {
    const fetchLockup = async () => {
      if (account) {
        fetch(
          'https://raw.githubusercontent.com/energyFiswap/energyFi-vesting/master/amounts-protocols-10959148-12171394.json'
        )
      ...
      <div className="relative w-full h-full overflow-hidden rounded bg-dark-900">
        <div className="flex flex-col gap-3 p-4">
          <div className="flex flex-row justify-between">
            <div className="font-bold text-white">{i18n._(t`Claimable Energyfi from Protocols`)}</div>
            <QuestionHelper text="If you participated in staking progammes from Alpha Homora, Cream, DefiDollar, Dracula, Harvest, Pickle, Yam, or Badger you can claim your vested Energyfi directly here" />
          </div>
          ...
          <div className="flex flex-col items-baseline pb-4">
            ...
            {account ? (
              <div className="text-sm text-secondary">
                {totalLocked ? (
                  i18n._(t`Historical Total Locked: ${formatNumber(totalLocked)} Energyfi`)
                ) : (
          ...
          >
            {pendingTreasurySignature ? (
              ...
              <> {claimConfirmed ? i18n._(t`Claimed`) : i18n._(t`Claim Energyfi`)}</>
            )}
...
const WeeklyVesting = () => {
  ...
  useEffect(() => {
    const fetchLockup = async () => {
      if (account) {
        fetch('https://raw.githubusercontent.com/energyFiswap/energyFi-vesting/master/amounts-10959148-12171394.json')
          .then((response) => response.json())
          .then((data) => {
            const userLockedAmount = data[account.toLowerCase()] ? data[account.toLowerCase()] : '0'
            const userLocked = Fraction.from(BigNumber.from(userLockedAmount), BigNumber.from(10).pow(18)).toString()
  ...
  if (!pendingTreasurySignature && Number(unclaimedAmount?.toFixed(8)) > 0) {
    vault = 'https://raw.githubusercontent.com/energyFiswap/energyFi-content/master/images/energyFi-vault-reverse.png'
  } else if (!pendingTreasurySignature && Number(unclaimedAmount?.toFixed(8)) <= 0) {
    vault = 'https://raw.githubusercontent.com/energyFiswap/energyFi-content/master/images/vesting-safe-off.png'
  } else if (pendingTreasurySignature) {
    vault = 'https://raw.githubusercontent.com/energyFiswap/energyFi-content/master/images/vesting-safe-closed.png'
  }
  ...
  return (
    <div className="flex flex-col gap-3 md:max-w-[400px]">
      <div className="relative w-full h-full overflow-hidden rounded bg-dark-900">
        <div className="flex flex-col gap-3 p-4">
          <div className="flex flex-row justify-between">
            <div className="font-bold text-white">{i18n._(t`Your Claimable Energyfi this Week`)}</div>
            <QuestionHelper text="Your Vested Energyfi will be released each week for the next 6 months. The amount released each week is determined by your historical farming rewards. You do not need to harvest each week as unclaimed amounts from each week will continue to accrue onto the next." />
          </div>
          {/* <div style={{ display: 'flex', alignItems: 'baseline' }}> */}
          <div className="flex flex-col items-baseline pb-4">
            <div className="font-bold text-white text-[36px]">
