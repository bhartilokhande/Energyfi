...
const MigrateButtons = ({ state, exchange }: { state: MigrateState; exchange: string | undefined }) => {
  ...
  return (
    <div className="space-y-4">
      {insufficientAmount ? (
        <div className="text-sm text-white">{i18n._(t`Insufficient Balance`)}</div>
      ) : state.loading ? (
        ...
        <>
          <div className="flex justify-between">
            <div className="text-sm text-secondary">
              {i18n._(t`Balance`)}:{' '}
              <span className="text-white">{state.selectedLPToken.balance.toSignificant(4)}</span>
            </div>
          </div>
          {state.mode === 'approve' && (
            <Button
      ...
      <div className="text-sm text-center text-low-emphesis">
        {i18n._(
          t`Your ${exchange} ${state.selectedLPToken.tokenA.symbol}/${state.selectedLPToken.tokenB.symbol} liquidity will become EnergyFiSwap ${state.selectedLPToken.tokenA.symbol}/${state.selectedLPToken.tokenB.symbol} liquidity.`
        )}
      </div>
    </div>
  )
...
export default function Migrate() {
  ...
  return (
    <Container id="migrate-page" className="py-4 space-y-6 md:py-8 lg:py-12" maxWidth="2xl">
      <Head>
        <title>Migrate | Energyfi</title>
        <meta key="description" name="description" content="Migrate your liquidity to EnergyFiSwap." />
        <meta key="twitter:description" name="twitter:description" content="Migrate your liquidity to EnergyFiSwap." />
        <meta key="og:description" property="og:description" content="Migrate your liquidity to EnergyFiSwap." />
      </Head>
      ...
      {!account ? (
        ...
        <div className="p-4 space-y-4 rounded bg-dark-900">
          {state.loading ? (
            <Typography variant="lg" className="p-4 text-center text-white">
