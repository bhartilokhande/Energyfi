  ...
  return (
    <PairLayout>
      <Head>
        ...
        <title>Lend {pair.asset.tokenInfo.symbol} | Energyfi</title>
        {/*@ts-ignore TYPE NEEDS FIXING*/}
      ...
      <Card
        ...
        header={
          <Card.Header className="border-b-8 border-Gray">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center">
                    ...
                    {/*@ts-ignore TYPE NEEDS FIXING*/}
                    <div className="mr-2 text-sm text-white">{pair && pair.collateral.tokenInfo.symbol}</div>
      ...
      >
        <div className="flex justify-between p-4 mb-8 xl:p-0">
          <div>
            ...
            {/*@ts-ignore TYPE NEEDS FIXING*/}
            <div className="text-lg text-white">{formatNumber(pair.currentUserAssetAmount.usd, true)}</div>
          </div>
          <div>
            <div className="text-lg text-secondary">{i18n._(t`Borrowed`)}</div>
            {/*@ts-ignore TYPE NEEDS FIXING*/}
            <div className="text-2xl text-white">{formatPercent(pair.utilization.string)}</div>
          </div>
          <div className="text-right">
            <div>
              <div className="text-lg text-secondary">{i18n._(t`Supply APR`)}</div>
              {/*@ts-ignore TYPE NEEDS FIXING*/}
              <div className="text-2xl text-white">{formatPercent(pair.supplyAPR.string)}</div>
            </div>
          </div>
        </div>

        <Tab.Group>
          <Tab.List className="flex p-1 rounded bg-ternary">
            <Tab
              className={({ selected }) =>
                `${
                  selected ? 'bg-ternary text-white' : ''
