...
const PoolDeposit = ({ currencyA, currencyB, header }) => {
  ...
  return (
    <>
      <HeadlessUiModal.BorderedContent className="flex flex-col gap-4 w-full h-full maxMd:overflow-auto">
        ...
        {(oneCurrencyIsETH || oneCurrencyIsWETH) && chainId != ChainId.CELO && (
          <div className="flex justify-center">
            <Button variant="outlined" className="rounded-[0.350rem] h-10 w-full text-white hover:bg-primary" onClick={() => setUseETH(!useETH)}>
      ...
      {!account ? (
        <Web3Connect fullWidth className='!rounded-[0.350rem] !font-normal text-white !h-10 !bg-primary text-white' />
      ) : isValid &&
        ...
        <Button
          ...
          color={!isValid && !!parsedAmounts[Field.CURRENCY_A] && !!parsedAmounts[Field.CURRENCY_B] ? 'btn_primary' : 'btn_primary'}
          onClick={() => {
            isExpertMode
              ? onAdd()
              : setContent(
