  ...
  if (onClick) {
    return (
      <Menu.Item>
        {({ active }) => {
          return (
            <Typography variant="sm" weight={700} onClick={onClick} className={active ? 'text-white' : 'text-white'}>
              {label}
            </Typography>
          )
        }}
...
const OnsenFilter = () => {
  ...
  const items = useMemo(() => {
    const map: Record<string, ReactNode> = {
      ...
      [FarmFilter.Energyfi]:
        chainId === ChainId.ETHEREUM ? (
          <MenuLink href={'/farm?filter=energyfi'} label={i18n._(t`EnergyfiSwap Farms`)} />
        ) : undefined,
  ...
  return (
    <div className="flex gap-2 items-center w-full md:w-[180px]">
      <Menu as="div" className="relative inline-block w-full text-left">
        <div >
          <Menu.Button className="w-full px-4 py-2.5 text-sm font-bold bg-field border rounded-[0.350rem] shadow-sm text-white border-Gray ">
