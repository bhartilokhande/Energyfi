...
const InvestmentDetails = ({ farm }) => {
  ...
  const rewardValue = !secondaryRewardOnly
    ? (farm?.rewards?.[0]?.rewardPrice ?? 0) * Number(pendingEnergyFi?.toExact() ?? 0) +
  ...
  return (
    <>
            ...
            <div className="flex items-center gap-2" key={i}>
              ...
              {!secondaryRewardOnly ? (
                <>
                  {i === 0 && (
                    <RewardRow
                      value={formatNumber(pendingEnergyFi?.toSignificant(6) ?? 0)}
      ...
      <Button
        ...
        disabled={
          pendingTx ||
          !((pendingEnergyFi && pendingEnergyFi.greaterThan(ZERO)) || (pendingReward && Number(pendingReward) > 0))
        }
        onClick={onHarvest}
      >
