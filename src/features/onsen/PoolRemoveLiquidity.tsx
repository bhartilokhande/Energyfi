    ...
    <>
      ...
      {!userLiquidity?.equalTo(ZERO) && (
        <Typography variant="xs" className="italic text-center">
          {i18n._(t`If your ELP is staked, you cannot remove your liquidity. You must unstake first.`)}
        </Typography>
      )}
      {!account ? (
        ...
        <Button
          ...
          color={!isValid && !!parsedAmounts[Field.CURRENCY_A] && !!parsedAmounts[Field.CURRENCY_B] ? 'red' : 'btn_primary'}
          onClick={() => {
            setContent(
              <PoolRemoveLiquidityReviewContent
                liquidityAmount={parsedAmounts[Field.LIQUIDITY]}
