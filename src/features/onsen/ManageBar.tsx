...
const ManageBar = ({ farm }) => {
  ...
  return (
    <>
          ...
          {['25', '50', '75', '100'].map((multiplier, i) => (
            <Button
              ...
              className={classNames(
                ...
                toggle ? 'focus:ring-Gray hover:ring-Gray border-Gray ' : '!bg-primary border-Gray',
                selectedTab === multiplier ? 'bg-Green border-Green border' : 'border-Gray !bg-transparent'
              )}
            >
      ...
      {toggle ? (
        !account ? (
          <Web3Connect className="w-full !rounded-[0.350rem] !font-normal text-white !h-10 !bg-primary text-white" />
        ) : isDepositValid &&
          ...
          <Button
            ...
            variant='filled'
            className='!rounded-[0.350rem] !font-normal text-white !h-10'
            color={!isDepositValid && !!parsedDepositValue ? 'red' : 'btn_primary'}
            onClick={async () => {
              try {
        ...
        <Button
          ...
          variant='filled'
          className='!rounded-[0.350rem] !font-normal text-white !h-10'
          color={!isWithdrawValid && !!parsedWithdrawValue ? 'red' : 'btn_primary'}
          onClick={async () => {
            try {
              // KMP decimals depend on asset, SLP is always 18
              // @ts-ignore TYPE NEEDS FIXING
