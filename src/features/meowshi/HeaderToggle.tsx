    ...
    <div className="flex justify-between">
      ...
      >
        <RadioGroup.Option
          value={true}
          className={({ checked }) =>
            classNames('px-8 py-2 rounded', `${checked ? 'bg-Greay' : ''}`)
          }
        >
          {({ checked }) => (
        ...
        <RadioGroup.Option
          value={false}
          className={({ checked }) =>
            classNames('px-8 py-2 rounded', `${checked ? 'bg-Green from-blue to-pink' : ''}`)
          }
        >
          {({ checked }) => (
            <Typography weight={checked ? 700 : 400} className={`${checked ? 'text-high-emphesis' : 'text-secondary'}`}>
      ...
      <div className="my-1.5 items-center flex border-Gray border-transparent border-solid border rounded-3xl px-4 md:px-3.5 py-1.5 md:py-0.5 text-high-emphesis text-xs font-medium md:text-base md:font-normal">
        1 xENERGYFI ≈ 100k MEOW
      </div>
    </div>
  )
}
