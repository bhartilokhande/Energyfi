...
import { BlocksIcon, MintableTokenIcon, ELogo } from 'app/components/Icon'
...
const TokenCreationFormTypeSelector: FC<TokenCreationFormTypeSelectorProps> = () => {
  ...
  const items = [
    ...
    {
      icon: <ELogo height={83} width={83} />,
      address: tokenTemplateMap?.[TokenType.ENERGYFI],
      value: TokenType.ENERGYFI,
      label: templateIdToLabel(TokenType.ENERGYFI),
      description: i18n._(
        t`Energyfi tokens function similar to mintable tokens but with additional capabilities built into the token. Creators will have to assign an owner address for token functions during minting.`
      ),
    },
  ]
