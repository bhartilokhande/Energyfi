  ...
  return (
    <TridentApproveGate inputAmounts={[parsedAmounts?.[0]]} tokenApproveOn={bentoBox?.address}>
      {({ approved, loading }) => {
        ...
        return (
          <div className="flex">
            <Button fullWidth id="wrap-button" loading={loading} color="btn_primary" className='!h-10 rounded-[0.350rem] text-white' varient="filled" disabled={disabled} onClick={execute}>
              {buttonText}
            </Button>
          </div>
        )
