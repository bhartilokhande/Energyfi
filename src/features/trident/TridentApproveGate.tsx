...
const TokenApproveButton: FC<TokenApproveButtonProps> = memo(
  ({ inputAmount, onStateChange, tokenApproveOn, id, onSLPPermit }) => {
    ...
    const { gatherPermitSignature, signatureData } = useTridentLiquidityTokenPermit(
      ['Uniswap V2', 'Energyfi LP Token'].includes(inputAmount?.currency.name ?? '') ? inputAmount?.wrapped : undefined,
      tokenApproveOn
    )
    ...
    if (!signatureData && [ApprovalState.NOT_APPROVED, ApprovalState.PENDING].includes(approveState)) {
      return (
        <Button fullWidth id={id} loading={approveState === ApprovalState.PENDING} color="btn_primary" onClick={handleApprove}>
    ...
    <div className="flex flex-col gap-3 mt-3 items-center">
      ...
      {inputAmounts.every((el) => el?.greaterThan(ZERO)) &&
        [BentoApprovalState.NOT_APPROVED, BentoApprovalState.PENDING].includes(approvalState) && (
          <Button
            className='!rounded-[0.350rem] border-[thin] !font-normal text-white !h-12 py-5 w-full'
            variant="outlined"
            color="green"
      ...
      {!account ? (
        ...
        <Button id="connect-wallet"
          ...
          variant="filled"
          className={classNames(className, '!border-none h-10 rounded-[0.350rem]')}
        >
