  ...
  volumeChange: {
    ...
    accessor: (row) => (
      <div>
        ...
        <div className="font-normal text-white">{formatNumber(row.volume1w, true, false, 2)}</div>
      </div>
    ),
    align: 'right',
  },
    ...
    Cell: (props) => (
      <div className="flex justify-end w-full h-full py-2 pr-2">
        <div className="w-32 h-10">
          <LineGraph data={props.value} stroke={{ solid: props.row.original.change1w >= 0 ? '#0FB871' : '#A466FF' }} />
        </div>
      </div>
    ),
    disableSortBy: true,
